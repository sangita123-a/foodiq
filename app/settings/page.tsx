  ...
  return (
    <main className="min-h-screen bg-background relative selection:bg-[var(--color-primary)] selection:text-white pt-[90px]">
      ...
      {activeSection !== "Delete Account" && (
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-border p-4 z-50 transform translate-y-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
          <div className="container mx-auto max-w-6xl px-4 md:px-8 flex items-center justify-between">
            <p className="text-gray-text font-bold hidden md:block">You have unsaved changes.</p>
            <div className="flex gap-4 w-full md:w-auto">
              <button className="flex-1 md:flex-none bg-section hover:bg-[#F1F5F9] text-foreground px-6 py-3 rounded-xl font-bold transition-colors border border-border">
                Cancel
              </button>
              <button type="submit" form="settings-form" className="flex-1 md:flex-none bg-primary hover:bg-primary-hover text-white px-8 py-3 rounded-xl font-bold transition-colors shadow-[0_0_20px_rgba(226, 55, 68,0.3)]">
                Save Changes
              </button>
            </div>
          </div>

    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 md:p-8 space-y-6">
      ...
      {toastAlert && (
        <div className="fixed top-6 right-6 z-50 max-w-md bg-red-600 border-2 border-white text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between gap-3 animate-bounce">
          ...
          <button onClick={() => setToastAlert(null)} className="text-white hover:text-red-200 text-lg font-bold">
            &times;
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-red-600/20 border border-red-500/40 flex items-center justify-center">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Live monitoring of delivery partner SOS incidents & real-time telemetry</p>
          </div>
        </div>

        <button
          onClick={fetchEmergencies}
          className="px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 font-semibold text-xs transition-colors flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 bg-slate-900/80 p-4 rounded-2xl border border-slate-800">
        ...
        <button
          ...
          className="bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl px-3 py-2 text-xs font-semibold transition-colors flex items-center justify-center gap-1.5"
        >
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-200 flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-red-500" /> Incident List ({totalCount})
              </h2>
              <span className="text-xs text-slate-400">Page {page} of {totalPages}</span>
            </div>

            {loading ? (
              <div className="text-center py-12 text-slate-500 text-xs flex justify-center">
                <RefreshCw className="w-5 h-5 animate-spin text-red-500" />
              </div>
            ) : emergencies.length === 0 ? (
              <div className="text-center py-12 text-slate-500 text-xs">No emergency incidents found matching filters.</div>
            ) : (
                  ...
                  <div
                    ...
                    className={`p-4 rounded-2xl border cursor-pointer transition-all ${
                      selectedEmergency?.id === e.id
                        ? "bg-slate-800/90 border-red-500/80 shadow-lg shadow-red-950/40"
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-sm text-slate-100">{e.reason}</span>
                          <span
                    ...
                    {e.order_number && (
                      <div className="mt-2.5 pt-2 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
                        ...
                        <span className="text-[10px] text-amber-400">{e.order_status}</span>
                      </div>
                    )}
                  </div>
                ))}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-3 border-t border-slate-800">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs text-slate-300 disabled:opacity-40"
                >
          ...
          {selectedEmergency ? (
            <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-2xl">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xl font-black text-red-400">{selectedEmergency.reason}</span>
                    <span
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Incident ID: {selectedEmergency.id}</p>
                </div>
                  ...
                  {selectedEmergency.status === "active" && (
                    <>
                      <button
                        ...
                        className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition-colors flex items-center gap-1.5 shadow-lg shadow-emerald-950/40"
                      >
                      ...
                      <button
                        ...
                        disabled={actionLoading}
                        className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-xs transition-colors flex items-center gap-1.5 shadow-lg shadow-red-950/40"
                      >
              ...
              <div className="h-64 w-full rounded-2xl overflow-hidden shadow-inner border border-slate-800">
                <GoogleEmergencyMap
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800/80">
                  <div className="text-slate-400">Rider Contact</div>
                  <a href={`tel:${selectedEmergency.partner_phone}`} className="font-bold text-emerald-400 hover:underline flex items-center gap-1 mt-0.5">
                    <Phone className="w-3.5 h-3.5" /> {selectedEmergency.partner_phone}
                  </a>
                </div>
                <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800/80">
                  <div className="text-slate-400">Battery Telemetry</div>
                  <div className="font-bold text-slate-200 mt-0.5 flex items-center gap-1">
                    <Battery className="w-3.5 h-3.5 text-blue-400" /> {selectedEmergency.battery_level ?? 85}%
                  </div>
                </div>
                <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800/80">
                  <div className="text-slate-400">Network Type</div>
                  <div className="font-bold text-slate-200 mt-0.5 flex items-center gap-1">
                    <Radio className="w-3.5 h-3.5 text-purple-400" /> {selectedEmergency.network_type || "4G"}
                  </div>
                </div>
                <div className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800/80">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                ...
                <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800/80 space-y-2">
                  <div className="font-bold text-slate-200 flex items-center gap-2 text-sm">
                    <User className="w-4 h-4 text-blue-400" /> Delivery Partner Info
                  </div>
                  <div className="space-y-1 text-slate-300">
                    <div><span className="text-slate-500">Name:</span> {selectedEmergency.partner_name}</div>
                    <div><span className="text-slate-500">Vehicle:</span> {selectedEmergency.vehicle_type || "Bike"} ({selectedEmergency.vehicle_number || "KA-01-AB-1234"})</div>
                    <a
                      href={`tel:${selectedEmergency.partner_phone}`}
                      className="inline-flex items-center gap-1.5 mt-2 px-3 py-1.5 rounded-xl bg-blue-600/20 text-blue-400 border border-blue-500/30 font-bold hover:bg-blue-600/30"
                    >
                ...
                <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800/80 space-y-2">
                  <div className="font-bold text-slate-200 flex items-center gap-2 text-sm">
                    <Store className="w-4 h-4 text-amber-400" /> Order & Customer Info
                  </div>
                  {selectedEmergency.order_number ? (
                    <div className="space-y-1 text-slate-300">
                      <div><span className="text-slate-500">Order:</span> #{selectedEmergency.order_number} ({selectedEmergency.order_status})</div>
                      <div><span className="text-slate-500">Customer:</span> {selectedEmergency.customer_name || "N/A"}</div>
                      {selectedEmergency.customer_phone && (
                        <a
                          href={`tel:${selectedEmergency.customer_phone}`}
                          className="inline-flex items-center gap-1.5 mt-2 px-3 py-1.5 rounded-xl bg-amber-600/20 text-amber-400 border border-amber-500/30 font-bold hover:bg-amber-600/30"
                        >
                    ...
                    <div className="text-slate-500 py-2">No active order was linked during this SOS.</div>
                  )}
                </div>
              </div>
              ...
              {selectedEmergency.description && (
                <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 text-xs space-y-1">
                  <div className="font-bold text-slate-300">Rider Description:</div>
                  <p className="text-slate-400 italic">"{selectedEmergency.description}"</p>
                </div>
              )}
            </div>
          ) : (
            <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-12 text-center text-slate-500 text-xs">
              Select an incident from the list to view live map tracking and details.
            </div>
          )}
        </div>
      </div>

        ...
        <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6">
          ...
          <div className="space-y-2">
            {(data?.top_dishes || []).map((d) => (
              <div key={d.id} className="flex justify-between text-sm border-b border-[#E5E7EB] py-2">
                <div>
            ...
            {!data?.top_dishes?.length && <p className="text-sm text-[#6B7280]">No data yet.</p>}
          </div>
        </div>
        <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6">
          ...
          <div className="space-y-2">
            {(data?.restaurant_performance || []).map((r) => (
              <div key={r.id} className="flex justify-between text-sm border-b border-[#E5E7EB] py-2">
                <div>
                  <p className="font-bold">{r.name}</p>
                  <p className="text-xs text-[#6B7280]">★ {r.rating || 0} · {r.orders} orders</p>
                </div>
      ...
      <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {(data?.customer_growth || []).map((w) => (
            <div key={w.week} className="bg-[#F8FAFC] border border-[#E5E7EB] rounded-xl px-4 py-3">
              <p className="text-xs text-[#6B7280]">{new Date(w.week).toLocaleDateString()}</p>
              <p className="font-black text-lg">+{w.users}</p>
            </div>
          ))}
          {!data?.customer_growth?.length && <p className="text-sm text-[#6B7280]">No growth data yet.</p>}
        </div>
      </div>
    </AdminShell>
  );

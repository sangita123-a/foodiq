        ...
        {tabs.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-bold transition ${
              tab === t.id ? "bg-primary text-white" : "bg-white border border-border text-gray-text"
            }`}
          >
            {t.label}
          </button>
          ...
          <form onSubmit={saveSeasonal} className="bg-white rounded-3xl border border-border p-6 space-y-4">
            ...
            {(["slug", "title", "subtitle", "banner_url", "offer_code"] as const).map((key) => (
              <input
                ...
                className="w-full border border-border rounded-xl px-4 py-3 text-sm capitalize"
              />
            ))}
            ...
            <div className="space-y-3">
              {seasonalList.map((s) => (
                <div key={s.id} className="border border-border rounded-xl p-4">
                  ...
                  <p className="text-xs text-gray-text">{s.subtitle}</p>
                  <p className="text-xs text-[#9CA3AF] mt-1">
                    {formatDate(s.starts_at)} → {formatDate(s.ends_at)}
                  </p>
                  {s.offer_code && (
                    <span className="inline-block mt-2 text-xs font-bold bg-primary/10 text-primary px-2 py-1 rounded">
                      {s.offer_code}
                    </span>
                  )}
                </div>

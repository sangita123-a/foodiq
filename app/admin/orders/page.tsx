  ...
  return (
    <AdminShell title="Order Management">
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-[#111827]">Orders</h1>
          <p className="text-[#6B7280]">Filter, assign riders, cancel, and refund.</p>
        </div>
        <div className="flex gap-3">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search orders…" className="bg-white border border-[#E5E7EB] rounded-xl px-4 py-2.5 text-sm" />
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} className="border-b border-[#E5E7EB]">
                  ...
                  <td className="p-4 text-sm text-[#6B7280]">{o.delivery_partner_name || "Unassigned"}</td>
                  <td className="p-4 font-black">{formatCurrency(o.total_amount)}</td>
                  <td className="p-4 text-right space-x-2">
                    <button type="button" onClick={() => openDetails(o.id)} className="text-xs font-bold text-[#111827]">Details</button>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-[#6B7280] mb-1">{String(selected.customer_name)} · {String(selected.restaurant_name)}</p>
            ...
            <div className="space-y-2 mb-4">
              {((selected.items as Array<{ name: string; quantity: number; price_at_time: number }>) || []).map((item, i) => (
                <div key={i} className="flex justify-between text-sm border-b border-[#E5E7EB] py-2">
            ...
            <div className="flex gap-2 mb-4">
              <select value={assignId} onChange={(e) => setAssignId(e.target.value)} className="flex-1 border border-[#E5E7EB] rounded-xl px-3 py-2 text-sm">
            ...
            <button type="button" onClick={() => setSelected(null)} className="w-full py-3 rounded-xl border border-[#E5E7EB] font-bold">Close</button>
          </div>
        </div>
      )}
    </AdminShell>

          ...
          <div className="max-w-7xl mx-auto space-y-6">
            <div>
              <h1 className="text-3xl font-black text-foreground flex items-center gap-2">
                <ChefHat className="w-8 h-8 text-primary" /> Kitchen Dashboard
              </h1>
              <p className="text-gray-text">Live order queue with prep timers.</p>
            </div>
            ...
            {isLoading && <p className="text-sm text-gray-text">Loading kitchen queue…</p>}

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              {COLUMNS.map((col) => {
                const colOrders = (data?.orders || []).filter((o) => col.statuses.includes(o.status));
                return (
                  <div key={col.key} className="bg-white rounded-3xl border border-border p-4 min-h-[320px]">
                    <h2 className="font-black text-foreground mb-3 text-sm uppercase tracking-wide">{col.label}</h2>
                    <div className="space-y-3">
                      {colOrders.map((order) => (
                        <div key={order.id} className="border border-border rounded-xl p-3">
                          ...
                          <p className="text-xs text-gray-text mb-2">{order.customer_name}</p>

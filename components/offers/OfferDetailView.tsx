      ...
      <div className="container mx-auto px-4 md:px-8 py-8 max-w-6xl">
        ...
        <div className={`relative rounded-3xl overflow-hidden bg-gradient-to-br ${offer.color_gradient || "from-rose-500 to-red-600"} mb-10`}>
          ...
          <div className="relative z-10 p-8 md:p-12">
            ...
            {offer.subtitle && (
              <p className="text-[#111827]/90 text-lg md:text-xl font-medium mb-6">{offer.subtitle}</p>
            )}
            <div className="flex flex-wrap gap-4">
              <div className="inline-flex items-center bg-[#F8FAFC] backdrop-blur-sm border border-white/30 rounded-lg px-4 py-2">
                <Tag className="w-4 h-4 text-[#111827] mr-2" />
                <span className="text-white font-bold">{offer.coupon_code}</span>
              </div>
              <div className="inline-flex items-center bg-[#F8FAFC] backdrop-blur-sm border border-white/30 rounded-lg px-4 py-2 text-white text-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-[#F8FAFC] rounded-2xl p-6 border border-[#E5E7EB]">
              ...
              <p className="text-[#6B7280] leading-relaxed">{offer.description}</p>
            </div>

            {offer.terms && (
              <div className="bg-[#F8FAFC] rounded-2xl p-6 border border-[#E5E7EB]">
                <h2 className="text-xl font-bold text-white mb-3">Terms & Conditions</h2>
                <p className="text-[#6B7280] text-sm leading-relaxed whitespace-pre-line">{offer.terms}</p>
              </div>
            )}
          </div>

          <div className="bg-[#F8FAFC] rounded-2xl p-6 border border-[#E5E7EB] h-fit">
                ...
                {restaurants.map((r: { id: string; name: string; rating: number }) => (
                  <li key={r.id}>
                    <Link
                      ...
                      className="flex items-center justify-between text-[#6B7280] hover:text-[#111827] transition-colors text-sm"
                    >
                      <span>{r.name}</span>
        ...
        <div className="mb-10">
          ...
          <p className="text-[#6B7280] mb-8">Add items to your cart — your offer coupon applies automatically at checkout.</p>
          ...
          {loadingItems ? (
            ...
            <div className="text-center py-16 bg-[#F8FAFC] rounded-2xl border border-[#E5E7EB]">
              <p className="text-[#6B7280]">No items available for this offer right now.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">

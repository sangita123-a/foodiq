        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {values.map((value, idx) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className="bg-white rounded-3xl p-8 border border-border hover:border-primary/40 transition-all duration-300 shadow-xl group text-center relative overflow-hidden"
            >
              ...
              <div className="relative z-10">
                <div className="w-16 h-16 mx-auto rounded-full bg-section border border-border flex items-center justify-center mb-6 group-hover:border-primary/50 transition-colors">
                  <value.icon className={`w-8 h-8 ${value.color}`} />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">{value.title}</h3>
                <p className="text-gray-text text-sm leading-relaxed">{value.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>

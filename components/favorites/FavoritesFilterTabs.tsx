...
export default function FavoritesFilterTabs({ activeTab, setActiveTab }: Props) {
  return (
    <div className="flex overflow-x-auto custom-scrollbar-hide mb-8 border-b border-border">
          ...
          return (
            <button
              ...
              className={`relative pb-4 text-lg font-bold transition-colors whitespace-nowrap ${
                isActive ? "text-foreground" : "text-[#9CA3AF] hover:text-foreground"
              }`}
            >
              {tab}
              {isActive && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary shadow-[0_0_10px_rgba(226, 55, 68,0.8)]"
                  initial={false}

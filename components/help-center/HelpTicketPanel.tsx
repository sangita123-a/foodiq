...
export default function HelpTicketPanel({ aiSessionId }: Props) {
  ...
  if (!hasToken) {
    return (
      <div className="bg-white rounded-3xl border border-[#E5E7EB] p-8 text-center">
        <p className="text-[#6B7280]">Sign in to create and track support tickets.</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <form onSubmit={submit} className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-4">
        <div className="flex items-center gap-2 mb-2">
          <Ticket className="w-5 h-5 text-[#E23744]" />
          <h2 className="text-lg font-black text-[#111827]">Create Ticket</h2>
        </div>
        <select
          value={form.category}
      ...
      <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6">
        <h2 className="text-lg font-black text-[#111827] mb-4">Your Tickets</h2>
        <div className="space-y-3 max-h-80 overflow-y-auto custom-scrollbar">
          {tickets.map((t) => (
            <div key={t.id} className="border border-[#E5E7EB] rounded-xl p-4">
              <div className="flex justify-between items-start gap-2 mb-1">
                ...
                <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-[#F8FAFC] text-[#6B7280]">
                  {t.status}
                </span>
              </div>
          ...
          {!tickets.length && <p className="text-sm text-[#6B7280]">No tickets yet.</p>}
        </div>
      </div>
    </div>
  );

  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {sessions.length === 0 ? (
            <div className="bg-[#111] border border-primary/20 rounded-2xl p-5 flex items-center gap-4">
            ...
            sessions.map((s: any) => (
              <div
                key={s.id}
                className={`bg-[#111] border rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                  s.is_current ? "border-primary/20" : "border-white/5"
                }`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      s.is_current ? "bg-primary/10" : "bg-white/5"
                    }`}
                  >
                    {s.device_type === "mobile" ? (
                      <Smartphone className={`w-6 h-6 ${s.is_current ? "text-primary" : "text-gray-400"}`} />
                    ) : (
                      <Laptop className={`w-6 h-6 ${s.is_current ? "text-primary" : "text-gray-400"}`} />
                    )}
                  </div>
                  <div>

...
export default function AppearanceSettings() {
  ...
  const accents = ["#0F766E", "#3B82F6", "#10B981", "#F59E0B", "#8B5CF6"];

  useEffect(() => {
    if (data) {
      setTheme((data.theme as any) || "dark");
      setAccent(data.accent_color || "#0F766E");
    }
  }, [data]);
              ...
              >
                <div
                  className={`w-12 h-12 rounded-full border border-border flex items-center justify-center ${
                    opt.id === "light" ? "bg-white" : "bg-background"
                  }`}
                >
                  <opt.icon className={`w-5 h-5 ${theme === opt.id ? "text-primary" : "text-gray-text"}`} />
                </div>
                <span className={`font-bold ${theme === opt.id ? "text-primary" : "text-gray-text"}`}>
                  {opt.label}
                </span>
              </button>
            ))}

...
export default function OrderCard({ order, onUpdateStatus, onViewDetails, isCompact = false }: OrderCardProps) {
  ...
  const renderActionButtons = () => {
    switch (order.status) {
      case "New":
        return (
          <div className="flex gap-2 w-full mt-4">
            ...
            <button onClick={() => onUpdateStatus(order.id, "Rejected")} className="flex-1 bg-section hover:bg-section text-red-400 border border-border py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-1">
      ...
      case "Accepted":
        return (
          <button onClick={() => onUpdateStatus(order.id, "Preparing")} className="w-full mt-4 bg-primary hover:bg-primary-hover text-white py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-1 shadow-lg shadow-primary/20">
  ...
  return (
    <motion.div
      ...
      className={`bg-background border rounded-2xl p-5 shadow-xl relative overflow-hidden group transition-colors ${
        isWaitingLong ? 'border-red-500/50 hover:border-red-500/80' : 'border-border hover:border-border'
      }`}
    >
      {/* Priority Pulse */}
      {isWaitingLong && (
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-mono font-black text-foreground text-lg">{order.id}</span>
            <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-md border ${statusColors[order.status]}`}>
              {order.status}
            </span>
          </div>
          <p className="text-gray-text text-xs font-bold">{order.orderTime}</p>
        </div>
        <div className="text-right">
          <p className="text-xl font-black text-foreground">₹{order.grandTotal}</p>
          <p className={`text-[10px] font-bold uppercase tracking-wider mt-0.5 ${order.paymentStatus === 'Paid' ? 'text-green-400' : 'text-primary'}`}>
      ...
      {!isCompact && (
        <div className="bg-section rounded-xl p-3 mb-4 border border-border space-y-2">
          <div className="flex items-center gap-2 text-foreground font-bold text-sm">
            {order.customerName}
          </div>
          <div className="flex items-center gap-2 text-gray-text text-xs">
            <Phone className="w-3.5 h-3.5 text-[#9CA3AF]" /> {order.customerPhone}
          </div>
          <div className="flex items-start gap-2 text-gray-text text-xs leading-relaxed">
        ...
        {order.items.slice(0, 2).map(item => (
          <div key={item.id} className="flex items-start justify-between text-sm">
            <div className="flex gap-2">
              ...
              <span className="text-gray-text font-medium truncate max-w-[180px]">{item.name}</span>
            </div>
          </div>
        ))}
      ...
      <div className="flex items-center justify-between mt-auto">
        <button onClick={() => onViewDetails(order)} className="text-xs font-bold text-gray-text hover:text-foreground transition-colors flex items-center gap-1">

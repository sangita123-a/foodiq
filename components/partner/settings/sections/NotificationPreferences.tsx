  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-400 text-sm mb-6">Control what alerts you receive across your devices.</p>
      </div>
      ...
      <div className="space-y-4">
        {options.map((opt) => (
          <div 
            key={opt.id}
            className="bg-[#111] border border-white/5 rounded-2xl p-5 flex items-center justify-between cursor-pointer group hover:border-white/20 transition-colors"
            onClick={() => onChange({ [opt.id]: !opt.value })}
          >
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center bg-[#171717] border border-white/10 group-hover:bg-white/5 transition-colors`}>
                <opt.icon className={`w-5 h-5 ${opt.color}`} />
              </div>
              <div>
                <h4 className="text-white font-bold mb-1">{opt.label}</h4>
                <p className="text-gray-400 text-xs hidden md:block">{opt.desc}</p>
              </div>
            </div>
            
            <div className={`w-12 h-6 rounded-full p-1 transition-colors ${opt.value ? 'bg-primary' : 'bg-[#171717] border border-white/10'}`}>
              <motion.div 
                layout

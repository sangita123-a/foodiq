...
function CircularProgress({ percentage, color, label, icon: Icon }: { percentage: number, color: string, label: string, icon: any }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-gray-400 text-xs font-bold uppercase tracking-wider text-center">{label}</span>
    </div>
  );
}
...
export default function OrderAnalytics({ data }: OrderAnalyticsProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div className="lg:col-span-2 bg-[#171717] rounded-3xl p-6 md:p-8 border border-white/5 shadow-xl">
        ...
        <div className="h-48 flex items-end justify-between gap-1 pt-4 border-b border-white/5 pb-2">
          {data.ordersByHour.map((d, idx) => (
            <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full group relative">
              <div className="absolute -top-8 opacity-0 group-hover:opacity-100 transition-opacity bg-[#111] border border-white/10 text-white text-[10px] font-bold px-2 py-1 rounded whitespace-nowrap z-20">
                {d.count} Orders
              </div>
              ...
              <motion.div 
                ...
                className={`w-full max-w-[20px] rounded-t-sm transition-colors ${d.hour === data.peakHour.split(' ')[0] ? 'bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.4)]' : 'bg-[#111] border-t border-x border-white/10 group-hover:bg-purple-500/50'}`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-between pt-2">
          ...
          {data.ordersByHour.filter((_, i) => i % 3 === 0).map((d, idx) => (
            <div key={idx} className="text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center w-8">
              {d.hour}
            </div>
          ))}
        </div>
      ...
      {/* Completion Rates */}
      <div className="bg-[#171717] rounded-3xl p-6 md:p-8 border border-white/5 shadow-xl flex flex-col">
        ...
        <div className="mt-8 bg-[#111] p-4 rounded-xl border border-white/5">

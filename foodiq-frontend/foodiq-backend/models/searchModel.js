...
const globalSearch = async (searchTerm) => {
  const query = `
    ...
    SELECT 'menu_item' as type, m.id, m.name, m.description, m.image_url,
           NULL::text as rating, COALESCE(m.discount_price, m.price) as price, m.is_vegetarian,
           r.name as restaurant_name, m.preparation_time
    FROM menu_items m
    JOIN restaurants r ON m.restaurant_id = r.id
    WHERE (m.name ILIKE $1 OR m.description ILIKE $1) AND m.is_available = true AND r.is_active = true

    UNION ALL

    SELECT 'category' as type, c.id, c.name, c.description, c.image_url,
